...
const { Owner, User, CollaborationType } = db;
...
cloudinary.config({
	...
	const create = async (req, res) => {
		try {
			...
			const record = await Owner.create({ ...req.body, imageUrl: imageurl });
	...
	const countOwner = async (req, res) => {
		try {
	...
	const toggleRestrict = async (req, res) => {
		try {
			const { id } = req.params;
			const record = await Owner.findByPk(id);
	...
	const readRestrictedCommunities = async (req, res) => {
		try {
			...
			const { count, rows: communities } = await Owner.findAndCountAll({
				where: {
					name: {
						[Op.iLike]: `%${search}%`
					},
	...
	const readId = async (req, res) => {
		try {
			const { id } = req.params;
			const record = await Owner.findOne({ where: { id },
				// include:{model:Owner, as:'owner'}
			 });
	...
	const readByUserId = async (req, res) => {
		try {
			...
			const communities = await Owner.findAll({ where: { user: userId } });
	...
	const update = async (req, res) => {
		try {
			...
			const [updated] = await Owner.update(updateData, { where: { id: req.params.id } });
			if (updated) {
...
module.exports = {create, readall, countOwner, toggleRestrict, readRestrictedCommunities, readId, update, deleteId, readByUserId};