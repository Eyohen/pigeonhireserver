...
// controller/analytics.js - Enhanced version
const { Request, Response } = require("express");
const db = require("../models");
const { Op } = require("sequelize");
const { UserAnalytics, ContactLog, ProfileView, Lead, Network, User, Community, Connector } = db;

// Get user analytics dashboard with detailed information
const getUserAnalytics = async (req, res) => {
  try {
    const { userId } = req.params;
    ...
    // Get detailed leads information
    const leadsDetails = await Lead.findAll({
      where: { userId },
      limit: parseInt(limit),
      offset: parseInt(offset),
      order: [["createdAt", "DESC"]],
      include: [
        {
          ...
          as: "community",
          attributes: [
            'id', 'name', 'email', 'description', 'communityType', 
            'location', 'state', 'size', 'website', 'whatsapp', 
            'telegram', 'twitter', 'rating', 'verified'
          ]
        },
        {
          model: Connector,
          ...
          attributes: [
            ...
            'description', 'communityName', 'website', 'whatsapp', 
            'telegram', 'twitter', 'linkedIn', 'verified'
          ]
        }
      ]
    });

    // Get detailed networks information
    const networksDetails = await Network.findAll({
      where: { userId },
      limit: parseInt(limit),
      offset: parseInt(offset),
      order: [["createdAt", "DESC"]],
    ...
    // Get detailed contact logs
    const contactsDetails = await ContactLog.findAll({
      where: { userId },
      limit: parseInt(limit),
      offset: parseInt(offset),
      order: [["createdAt", "DESC"]],
    ...
    const profileViewsDetails = await ProfileView.findAll({
      where: { userId },
      limit: parseInt(limit),
      offset: parseInt(offset),
      order: [["createdAt", "DESC"]],
    ...
    const totalNetworks = await Network.count({ where: { userId } });
    const totalContacts = await ContactLog.count({ where: { userId } });
    const totalProfileViews = await ProfileView.count({ where: { userId } });
    ...
    return res.status(200).json({
      ...
      detailedData: {
        ...
        networks: {
          data: networksDetails,
          totalCount: totalNetworks,
          totalPages: Math.ceil(totalNetworks / limit),
          currentPage: parseInt(page)
        },
        contacts: {
        ...
        profileViews: {
          data: profileViewsDetails,
          totalCount: totalProfileViews,
          totalPages: Math.ceil(totalProfileViews / limit),
          currentPage: parseInt(page)
        }
      }
    });
